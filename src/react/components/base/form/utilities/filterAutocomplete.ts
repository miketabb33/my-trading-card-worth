...
export const filterAutocomplete = <T extends object>(options: DropdownOption<T>[] | null, newValue: string) => {
  if (!options) return []
  if (newValue === '') return options
  else return filterAutocompleteFromInput(options, newValue)
}

const filterAutocompleteFromInput = <T extends object>(options: DropdownOption<T>[], inputValue: string) => {
